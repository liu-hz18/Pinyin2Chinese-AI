...
def vitebi_2gram(sentence, outputfile, style='modern'):
    ...
    if len(pinyin_list) < 2:
        print("Cannot input a singel pinyin!")
    ...
    for i in range(len(candidate)):
        dp[0][i] = weightmat[vocab2id[("</bos>", "")]][vocab2id[(candidate[i], first_py)]]
    ...
    memlist = []
    for i in range(len(candidate)):
        cur = vocab2id[(candidate[i], pinyin_list[1])]
        for j in range(len(lastcandidate)):
            last = vocab2id[(lastcandidate[j], first_py)]
    ...
    t = 2
    lastlastcandidate, lastlastpinyin = lastcandidate, first_py
    lastcandidate, lastpinyin = candidate, pinyin_list[1]
    for pinyin in pinyin_list[2:]:
        candidate = pinyin2char[pinyin]
        memlist = []
        for i in range(len(candidate)):
            flagj = 0
            cur = vocab2id[(candidate[i], pinyin)]
            for j in range(len(lastcandidate)):
                last = vocab2id[(lastcandidate[j], lastpinyin)]
                factor = weightmat[last][cur]
                for k in range(len(lastlastcandidate)):
                    lastlast = vocab2id[(lastlastcandidate[k], lastlastpinyin)]
                    if (lastlast, last, cur) in count_3gram:
            ...
            memlist.append(flagj)
        mem.append(memlist)
        lastlastcandidate, lastlastpinyin = lastcandidate, lastpinyin
        lastcandidate, lastpinyin = candidate, pinyin
        t += 1
    # 处理</eos>
    flagj = 0
    for j in range(len(lastcandidate)):
...
def vitebi_3gram(sentence, outputfile):
    ...
    candidate = pinyin2char[first_py]
    for i in range(len(candidate)):
        dp[0][0][i] = weightmat[vocab2id[("</bos>", "")]][vocab2id[(candidate[i], first_py)]]
    # 从list[1]到</eos>
    t = 1
    lastlastcandidate, lastlastpinyin = ['</bos>'], ""
    lastcandidate, lastpinyin = pinyin2char[pinyin_list[t-1]], pinyin_list[t-1]
